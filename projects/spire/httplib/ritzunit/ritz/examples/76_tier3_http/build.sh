#!/bin/bash
# Build Tier 3 HTTP server
set -e

LANGDEV=/home/aaron/dev/nevelis/langdev
TIER3=/home/aaron/dev/nevelis/langdev/examples/76_tier3_http

export RITZ_PATH=$LANGDEV

# Parse arguments
OPT_LEVEL="-O2"
BUILD_TYPE="release"
while [[ $# -gt 0 ]]; do
    case $1 in
        --debug|-d)
            OPT_LEVEL="-O0"
            BUILD_TYPE="debug"
            shift
            ;;
        --release|-r)
            OPT_LEVEL="-O2"
            BUILD_TYPE="release"
            shift
            ;;
        --profile|-p)
            OPT_LEVEL="-O2 -g"
            BUILD_TYPE="profile"
            shift
            ;;
        *)
            echo "Unknown option: $1"
            echo "Usage: ./build.sh [--debug|-d] [--release|-r] [--profile|-p]"
            exit 1
            ;;
    esac
done

echo "Building tier3_http ($BUILD_TYPE mode, $OPT_LEVEL)..."

mkdir -p $TIER3/.build

# Get all dependencies
DEPS=$(python3 $LANGDEV/ritz0/list_deps.py $TIER3/src/main.ritz)

# Compile each dependency to LLVM IR and object file
OBJECTS=""
for DEP in $DEPS; do
    BASENAME=$(basename $DEP .ritz)
    OUT=$TIER3/.build/$BASENAME

    echo "Compiling $BASENAME..."

    # Check if it's the main file (needs runtime) or a library
    if [[ "$DEP" == *"/src/main.ritz" ]]; then
        python3 $LANGDEV/ritz0/ritz0.py $DEP -o $OUT.ll
    else
        python3 $LANGDEV/ritz0/ritz0.py $DEP -o $OUT.ll --no-runtime
    fi

    clang-19 -c $OPT_LEVEL $OUT.ll -o $OUT.o -Wno-override-module
    OBJECTS="$OBJECTS $OUT.o"
done

# Link all objects (nostdlib because Ritz provides its own _start)
echo "Linking tier3_http..."
clang $OBJECTS -o $TIER3/tier3_http -nostdlib -no-pie

echo "Build complete: $TIER3/tier3_http ($BUILD_TYPE)"
ls -la $TIER3/tier3_http
