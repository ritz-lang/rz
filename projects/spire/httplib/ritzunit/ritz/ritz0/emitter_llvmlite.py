...
class StructRegistry:
    ...
    def declare(self, name: str, fields: List[Tuple[str, rast.Type]]) -> None:
...
class LLVMEmitter:
            ...
            if isinstance(item, rast.ConstDef):
                ...
                if isinstance(item.value, rast.IntLit):
                    ...
                    if isinstance(item.value.operand, rast.IntLit):
                        ...
                        raise ValueError(f"Const value must be int literal: {item.value}")
                    ...
                    raise ValueError(f"Const value must be int literal or array literal: {item.value}")
    ...
    def _declare_struct(self, struct_def: rast.StructDef):
        ...
        self.struct_registry.declare(struct_def.name, struct_def.fields)
        ...
        struct_type = self.module.context.get_identified_type(unique_name)
    ...
    def _declare_function(self, fn_def: rast.FnDef) -> ir.Function:
        ...
        # Build function type
        param_types = [self._ritz_type_to_llvm(p.type) for p in fn_def.params]
  ...
  """
    ...
    def _emit_array_fill(self, expr: rast.ArrayFill) -> ir.Value:
        ...
        # Build array aggregate by inserting the same value at each position
        result = ir.Constant(arr_type, ir.Undefined)
    ...
    def _emit_lvalue_addr(self, expr: rast.Expr) -> ir.Value:
        ...
        if isinstance(expr, rast.Ident):
            ...
            if name in self.locals:
                ...
                raise ValueError(f"Cannot get address of immutable binding: {name}")
            else:
                raise ValueError(f"Unknown variable: {name}")
        elif isinstance(expr, rast.UnaryOp) and expr.op == '*':
    ...
    def _emit_unary(self, expr: rast.UnaryOp) -> ir.Value:
        """Emit a unary operation: -x, !x, *x, &x."""
        # Handle & specially - we need the address, not the value
        if expr.op == '&':
            if isinstance(expr.operand, rast.Ident):
                ...
                if name in self.locals:
                    ...
                    # For params that are values, we'd need to create a temp alloca
                    raise ValueError(f"Cannot take address of immutable binding: {name}")
                else:
                    raise ValueError(f"Unknown variable: {name}")
