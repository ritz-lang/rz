...
class Parser:
    ...
    def parse_param(self) -> rast.Param:
        ...
        param_type = self.parse_type()
        return rast.Param(span, name_tok.value, param_type)
    ...
    def parse_struct(self, is_pub: bool = False) -> rast.StructDef:
    ...
    def parse_item(self) -> rast.Item:
        ...
        if self._at(TokenType.STRUCT):
            if attrs:
                raise ParseError("Attributes not allowed on structs (yet)", self._current().span)
            return self.parse_struct(is_pub=is_pub)
