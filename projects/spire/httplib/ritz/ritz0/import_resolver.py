...
class ImportResolver:
    ...
    def __init__(
        ...
        self.functions: Dict[str, rast.FnDef] = {}
        self.extern_fns: Dict[str, rast.ExternFn] = {}
        self.constants: Dict[str, rast.ConstDef] = {}
    ...
    def _register_item(self, item: rast.Item, source_file: str):
        ...
        if isinstance(item, rast.StructDef):
            ...
            if item.name in self.functions:
                existing = self.functions[item.name]
                # For now, just warn about duplicates - last one wins
                # This allows file concatenation to still work
                pass
            ...
            if item.name in self.constants:
                existing = self.constants[item.name]
                # Allow duplicate if same value
