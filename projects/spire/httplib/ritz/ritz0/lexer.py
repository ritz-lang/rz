...
RERITZ Mode:
    When reritz_mode=True, the lexer recognizes new syntax:
    - [[ and ]] for attributes (instead of @name)
    - :& for mutable borrow parameters
    - := for move/ownership parameters
    - @& for mutable reference types
    - *& for mutable raw pointer types
    - @ means address-of (not attribute prefix)
...
# Global RERITZ mode flag - can be set via environment variable or programmatically
RERITZ_MODE = os.environ.get('RITZ_SYNTAX', 'legacy') == 'reritz'
...
class Lexer:
    ...
    def __init__(self, source: str, filename: str = "<stdin>", reritz_mode: bool = None):
        self.source = source
        ...
        # RERITZ mode - use parameter if provided, otherwise fall back to global
        self.reritz_mode = reritz_mode if reritz_mode is not None else RERITZ_MODE
    ...
    def _lex_operator(self) -> Token:
        ...
        # [[ and ]] for attributes - works in BOTH legacy and RERITZ modes
        # This allows gradual migration: [[test]] works everywhere
        if ch == '[' and next_ch == '[':
            self._advance()
            return Token(TokenType.LBRACKET2, '[[', start_span)
        if ch == ']' and next_ch == ']':
            self._advance()
            return Token(TokenType.RBRACKET2, ']]', start_span)

        # RERITZ: Additional two-character tokens (RERITZ mode only)
        if self.reritz_mode:
            # :& - mutable borrow parameter
            if ch == ':' and next_ch == '&':
                self._advance()
                return Token(TokenType.COLON_AMP, ':&', start_span)
            # := - move/ownership parameter
            if ch == ':' and next_ch == '=':
                self._advance()
                return Token(TokenType.COLON_EQ, ':=', start_span)
            # @& - mutable reference type / take mutable reference
            if ch == '@' and next_ch == '&':
                self._advance()
                return Token(TokenType.AT_AMP, '@&', start_span)
            # *& - mutable raw pointer type
            if ch == '*' and next_ch == '&':
                self._advance()
                return Token(TokenType.STAR_AMP, '*&', start_span)
...
def tokenize(source: str, filename: str = "<stdin>", reritz_mode: bool = None) -> List[Token]:
    """Convenience function to tokenize source code.

    Args:
        source: The source code to tokenize
        filename: The filename for error reporting
        reritz_mode: If True, use RERITZ syntax. If None, use global RERITZ_MODE.
    """
    lexer = Lexer(source, filename, reritz_mode=reritz_mode)
    return lexer.tokenize()


def set_reritz_mode(enabled: bool) -> None:
    """Set the global RERITZ mode flag.

    This affects all lexers created without an explicit reritz_mode parameter.
    """
    global RERITZ_MODE
    RERITZ_MODE = enabled
