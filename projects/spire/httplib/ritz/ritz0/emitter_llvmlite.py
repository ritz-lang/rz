...
class LLVMEmitter:
    ...
    def _infer_ritz_type(self, expr: rast.Expr, llvm_val: ir.Value = None) -> Optional[rast.Type]:
        ...
        if isinstance(expr, rast.IntLit):
            ...
            # String literals are *u8 pointers (C-strings)
            # Use c"..." syntax for explicit C-strings
    ...
    def _process_trait(self, trait_def: rast.TraitDef):
        ...
        With monomorphization, traits are compile-time contracts - no runtime vtables.
        """
    ...
    def _declare_extern(self, extern: rast.ExternFn) -> ir.Function:
        """Declare an external function (used for type info, actual calls are syscalls)."""
        ...
        fn_type = ir.FunctionType(ret_type, param_types)
        # Don't actually declare it - we inline syscalls
    ...
    def _emit_function_body(self, fn_def: rast.FnDef) -> ir.Function:
        ...
        for arg, param in zip(fn.args, fn_def.params):
            ...
            # Check if parameter is mutable borrow (:& in RERITZ)
            # Only MUTABLE borrows pass by reference for now
            is_mutable_borrow = hasattr(param, 'borrow') and param.borrow == rast.Borrow.MUTABLE

            if is_mutable_borrow:
                # Mutable borrow parameter: arg is already a pointer, use it directly
                # The pointee type is what we store in locals
                pointee_type = arg.type.pointee
                self.locals[param.name] = (arg, pointee_type)
                # Store Ritz type for enum/union detection
  ...
  """
    ...
    def _emit_asm(self, stmt: rast.AsmStmt):
        ...
        Becomes LLVM IR:
            call void asm sideeffect "mov dx, $0\0Aout dx, al", "r"(i16 %port)
        """
        import re

        # Convert {name} placeholders to LLVM $N format
        template = stmt.template

        # Build constraint string and operand list
        constraints = []
        operand_values = []
        ...
        for i, operand in enumerate(stmt.operands):
            ...
            # Determine constraint based on type
            # For now, use "r" (general register) for most things
            if operand.is_output:
                constraints.append("=r")
            else:
                # Input constraint - use type-appropriate register class
                ty = val.type
                if ty in (self.i8, self.i16, self.i32, self.i64):
                    constraints.append("r")  # General purpose register
                elif isinstance(ty, ir.PointerType):
                    constraints.append("r")  # Pointer in register
                else:
                    constraints.append("r")  # Default to register

        # Replace {name} with $N in template
        def replace_operand(match):
            name = match.group(1)
            if name in operand_map:
                return operand_map[name]
            else:
                raise ValueError(f"Unknown asm operand: {{{name}}}")

        llvm_template = re.sub(r'\{(\w+)\}', replace_operand, template)

        # Convert newlines to \0A for LLVM inline asm
        llvm_template = llvm_template.replace('\n', '\\0A')

        # Note: LLVM inline asm uses AT&T syntax by default for x86_64.
        # We could wrap with .intel_syntax noprefix ... .att_syntax, but this
        # conflicts with LLVM's operand substitution ($N references).
        # For now, asm blocks without operands can use Intel syntax by wrapping
        # manually, but blocks with operands should use AT&T syntax.

        # Auto-detect register clobbers from the template
        # This scans for x86_64 register names used in the assembly
        x86_64_regs = {
            # 64-bit
        ...
        # Find all register references in the template
        detected_clobbers = set()
        template_lower = template.lower()
        for reg in x86_64_regs:
            # Match register as a word boundary (avoid matching partial names)
            if re.search(rf'\b{reg}\b', template_lower):
                # Map to canonical 64-bit register name for clobbering
                canonical = reg
                # Map partial registers to their full 64-bit names
                reg_map = {
        ...
        # Determine function type for inline asm
        # For now, all asm is void-returning
        if operand_values:
            param_types = [v.type for v in operand_values]
            asm_fn_type = ir.FunctionType(self.void, param_types)
        else:
            asm_fn_type = ir.FunctionType(self.void, [])

        # Create inline asm value
        # llvmlite supports inline asm via module.add_inline_asm or builder.asm
        # We'll use the inline asm call approach
        ...
        # Call the inline asm
        if operand_values:
            self.builder.call(asm_val, operand_values)
        else:
            self.builder.call(asm_val, [])
    ...
    def _emit_expr(self, expr: rast.Expr) -> ir.Value:
        ...
        if isinstance(expr, rast.IntLit):
            ...
            # String literals are emitted as C-strings (*u8) by default
            # (avoiding the need to import ritzlib.string for simple use cases)
            # Use string_from_bytes() only when explicitly converted to String type
            gvar = self._get_string_constant(expr.value)
    ...
    def _emit_unary(self, expr: rast.UnaryOp) -> ir.Value:
        """Emit a unary operation: -x, !x, *x, &x, @x, @&x."""
        # Handle address-of operators: &, @, &mut, @&
        # In RERITZ mode: @ = immutable ref, @& = mutable ref
        # In legacy mode: & = immutable ref, &mut = mutable ref
        ...
        if expr.op in ('&', '@', '&mut', '@&'):
            if isinstance(expr.operand, rast.Ident):
                ...
                # For &arr[i], we need to get the pointer to the element, not load it
                index = self._emit_expr(expr.operand.index)

                # Special handling for identifiers that are arrays
                if isinstance(expr.operand.expr, rast.Ident):
                    name = expr.operand.expr.name
                    if name in self.locals:
                        alloca, ty = self.locals[name]
                        if isinstance(ty, ir.ArrayType):
                            # Array type - use two-level GEP [0, index] on the alloca
                ...
                # Check if the base is a field access (e.g., defs.defs[i])
                if isinstance(expr.operand.expr, rast.Field):
                    field_addr = self._emit_lvalue_addr(expr.operand.expr)
                    if isinstance(field_addr.type, ir.PointerType):
                        pointee = field_addr.type.pointee
                        if isinstance(pointee, ir.ArrayType):
                            # Field is an array - GEP with [0, index]
                            zero = ir.Constant(self.i32, 0)
                            ptr = self.builder.gep(field_addr, [zero, index])
                            return ptr

                # General case - pointer indexing
                base = self._emit_expr(expr.operand.expr)
                if isinstance(base.type, ir.PointerType):
                    ptr = self.builder.gep(base, [index])
                    return ptr
                else:
                    raise ValueError(f"Cannot take address of index on type: {base.type}")
    ...
    def _emit_call(self, call: rast.Call) -> ir.Value:
        ...
        if fname.startswith('syscall') or fname.startswith('__syscall'):
        ...
        if fname in self.functions:
            ...
            for i, arg in enumerate(call.args):
                ...
                # Check if this parameter is mutable borrow (:& in RERITZ)
                # Only MUTABLE borrows pass by reference for now
                is_mutable_borrow = hasattr(param_def, 'borrow') and param_def.borrow == rast.Borrow.MUTABLE

                if is_mutable_borrow:
                    # Mutable borrow parameter: emit address of the argument
...
def emit(module: rast.Module, no_runtime: bool = False,
    ...
    emitter = LLVMEmitter(source_file=source_file, source_dir=source_dir)
