...
# ============================================================================
# RERITZ Syntax Tests
# These tests verify the new RERITZ syntax parses correctly when reritz_mode=True
# ============================================================================

def reritz_parse(source: str):
    """Helper to parse in RERITZ mode."""
    tokens = tokenize(source, reritz_mode=True)
    parser = Parser(tokens, reritz_mode=True)
...
class TestReritzBackwardsCompatibility:
    """Verify legacy syntax still works in non-RERITZ mode."""

    def test_legacy_attribute(self):
        """@test should still work in legacy mode"""
        mod = parse("@test\nfn test_foo()\n  0")
        fn = mod.items[0]
        assert fn.has_attr("test")

    def test_legacy_ref_type(self):
        """&T should still work in legacy mode"""
    ...
    def test_legacy_mut_ref_type(self):
        """&mut T should still work in legacy mode"""
        mod = parse("fn foo(x: &mut User)\n  0")
        fn = mod.items[0]
        param_type = fn.params[0].type
        assert isinstance(param_type, rast.RefType)
        assert param_type.mutable is True

    def test_legacy_address_of(self):
        """&x should still work in legacy mode"""
        mod = parse("fn main()\n  &x")
        fn = mod.items[0]
        expr = fn.body.expr
        assert isinstance(expr, rast.UnaryOp)
