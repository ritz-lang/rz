...
RERITZ Mode:
    When reritz_mode=True, the parser recognizes new syntax:
    - [[attr]] instead of @attr for attributes
...
import lexer as lexer_module  # For accessing RERITZ_MODE dynamically
...
class Parser:
    ...
    RERITZ mode enables new syntax features. See RERITZ.md for details.
    """

    def __init__(self, tokens: List[Token], reritz_mode: bool = None):
        self.tokens = tokens
        self.pos = 0
        # RERITZ mode - use parameter if provided, otherwise fall back to global from lexer
        # Note: We access lexer_module.RERITZ_MODE dynamically to pick up changes from set_reritz_mode()
        self.reritz_mode = reritz_mode if reritz_mode is not None else lexer_module.RERITZ_MODE
    ...
    def _parse_type_primary(self, allow_type_args: bool = True) -> rast.Type:
        ...
        # RERITZ: @&T (mutable reference) - must check before @T
        if self.reritz_mode and self._at(TokenType.AT_AMP):
            self._advance()
            inner = self._parse_type_primary(allow_type_args)
            return rast.RefType(span, inner, mutable=True)

        # RERITZ: @T (immutable reference)
        if self.reritz_mode and self._at(TokenType.AT):
            self._advance()
            inner = self._parse_type_primary(allow_type_args)
            return rast.RefType(span, inner, mutable=False)

        # Legacy reference types: &T, &mut T
        if self._at(TokenType.AMP):
            ...
            if self._at(TokenType.MUT):
                ...
                mutable = True
            inner = self._parse_type_primary(allow_type_args)
            return rast.RefType(span, inner, mutable)

        # RERITZ: *&T (mutable raw pointer) - must check before *T
        if self.reritz_mode and self._at(TokenType.STAR_AMP):
            self._advance()
            inner = self._parse_type_primary(allow_type_args)
    ...
    def parse_prefix(self) -> rast.Expr:
        ...
        # RERITZ: @& for taking mutable reference (address-of mutable)
        if self.reritz_mode and self._at(TokenType.AT_AMP):
            self._advance()
            operand = self.parse_prefix()
            return rast.UnaryOp(span, '@&', operand)

        # RERITZ: @ for taking immutable reference (address-of)
        if self.reritz_mode and self._at(TokenType.AT):
            self._advance()
            operand = self.parse_prefix()
            return rast.UnaryOp(span, '@', operand)
        ...
        # Unary operators
        if self._at(TokenType.MINUS, TokenType.BANG, TokenType.STAR, TokenType.AMP, TokenType.TILDE):
            op = self._advance()
            # For &mut
            if op.type == TokenType.AMP and self._at(TokenType.MUT):
                self._advance()
                operand = self.parse_prefix()
                return rast.UnaryOp(span, '&mut', operand)
            operand = self.parse_prefix()
            ...
            if self._at(TokenType.LPAREN):
                ...
                if not self._at(TokenType.RPAREN):
                    ...
                    while self._at(TokenType.COMMA):
                        ...
                        fields.append(self.parse_pattern())
                self._expect(TokenType.RPAREN)
                return rast.VariantPattern(span, name, fields)
    ...
    def parse_asm(self) -> rast.AsmStmt:
        ...
        while not self._at(TokenType.DEDENT, TokenType.EOF):
            ...
            if line_parts:
                ...
                line = re.sub(r'\[\s*', '[', line)
                line = re.sub(r'\s*\]', ']', line)
                # Clean up $$ immediate escaping (no space between $s)
                line = re.sub(r'\$\s+\$', '$$', line)
                # Clean up % register prefix spacing
                line = re.sub(r'%\s+', '%', line)
                # Clean up $ immediate prefix spacing
    ...
    def parse_param(self) -> rast.Param:
    ...
    def parse_attrs(self) -> List[rast.Attr]:
        ...
        Legacy syntax: @attr, @attr2
        RERITZ syntax: [[attr]], [[attr, attr2]], [[attr("arg")]]

        Note: [[attr]] is accepted in BOTH modes for gradual migration.
        """
        attrs = []

        # Parse [[attr]] syntax (works in both legacy and RERITZ modes)
        while self._at(TokenType.LBRACKET2):
            self._advance()  # consume [[
            # Parse comma-separated attributes
            if not self._at(TokenType.RBRACKET2):
                ...
                while self._at(TokenType.COMMA):
                    ...
                    if self._at(TokenType.RBRACKET2):
                        break
                    attrs.append(self._parse_single_attr())
            self._expect(TokenType.RBRACKET2, "Expected ]] to close attribute")
            self._skip_newlines()

        # Also parse @attr in legacy mode (for backwards compatibility)
        if not self.reritz_mode:
            while self._at(TokenType.AT):
                self._advance()  # consume @
                name_tok = self._expect(TokenType.IDENT, "Expected attribute name after @")
                attrs.append(rast.Attr(name_tok.value))
                self._skip_newlines()  # Skip newlines after attribute

        return attrs

    def _parse_single_attr(self) -> rast.Attr:
        """Parse a single attribute: name or name("arg", ...)."""
            ...
            if not self._at(TokenType.RPAREN):
                ...
                while self._at(TokenType.COMMA):
                    ...
                    if self._at(TokenType.STRING):
                        args.append(self._advance().value)
            self._expect(TokenType.RPAREN)
        return rast.Attr(name_tok.value, args)

    def parse_fn(self, attrs: List[rast.Attr] = None, is_async: bool = False, is_pub: bool = False) -> rast.FnDef:
    ...
    def parse_const(self, is_pub: bool = False) -> rast.ConstDef:
        ...
        value = self.parse_expr()
        return rast.ConstDef(span, name, const_type, value, is_pub=is_pub)
    ...
    def parse_impl(self) -> rast.ImplBlock:
        ...
        if self._at(TokenType.LT):
            ...
            impl_type = rast.NamedType(first_name, type_args)
        ...
        if self._at(TokenType.FOR):
            ...
            if self._at(TokenType.LT):
                type_args = self._parse_type_args()
                impl_type = rast.NamedType(type_name, type_args)
        ...
        while not self._at(TokenType.DEDENT, TokenType.EOF):
            ...
            if self._at(TokenType.ASYNC):
                self._advance()
                is_async = True
            method = self.parse_fn(attrs, is_async=is_async, is_pub=method_is_pub)
            methods.append(method)
            self._skip_newlines()
    ...
    def parse_item(self) -> rast.Item:
        ...
        if self._at(TokenType.CONST):
            if attrs:
                raise ParseError("Attributes not allowed on constants (yet)", self._current().span)
