...
# ============================================================================
# RERITZ Syntax Tests
# These tests verify the new RERITZ tokens work correctly when reritz_mode=True
# ============================================================================

def reritz_token_types(source: str) -> list[TokenType]:
    """Helper to get token types in RERITZ mode."""
    return [t.type for t in tokenize(source, reritz_mode=True)]


def reritz_token_values(source: str) -> list[tuple[TokenType, any]]:
    """Helper to get token types and values in RERITZ mode."""
    return [(t.type, t.value) for t in tokenize(source, reritz_mode=True)]
...
class TestReritzAttributes:
    ...
    def test_double_bracket_attribute(self):
        """[[test]] should tokenize as LBRACKET2 + IDENT + RBRACKET2"""
...
class TestReritzParameterModifiers:
    ...
    def test_legacy_colon_remains_separate(self):
        """In legacy mode, : and & are separate tokens"""
        tokens = token_values(":&")  # Legacy mode
        assert tokens == [
            (TokenType.COLON, ":"),
            (TokenType.AMP, "&"),
            (TokenType.EOF, None),
    ...
    def test_legacy_colon_eq_separate(self):
        """In legacy mode, : and = are separate tokens"""
        tokens = token_values(":=")  # Legacy mode
        assert tokens == [
            (TokenType.COLON, ":"),
...
fn test_foo()"""
    ...
    def test_inline_attribute(self):
        ...
        assert TokenType.RBRACKET2 in tokens
        assert TokenType.FN in tokens
