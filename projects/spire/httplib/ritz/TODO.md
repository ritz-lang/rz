...
| dual-compiler tests | **34/36 passing for ritz1** (after pointer/i64 fixes) |
| Tier 1 examples | 11/11 compile with ritz1 |
| Tier 2 (generics) | Blocked on monomorphization |

**Recent Fixes (2026-02-13):**
- ✅ Implemented struct monomorphization in ritz1 (Pair<T> → Pair$i32)
- ✅ Added generic struct literal parsing (Pair<i32> { first: 1, second: 2 })
- ✅ Generic structs now compile and run correctly (test: exit code 42 = 10 + 32)
- Fixed function call type mismatch (pointers passed as i64 now match signatures)
- Fixed pointer variable load/store consistency (all use i64 uniformly)
- Added ritz_crt0.o linking for dual-compiler tests
- 2 remaining failures: u32 type not supported, ritzlib lexer errors
...
1. **Generics monomorphization** (#72) - 🔄 PARTIALLY COMPLETE
   - ✅ ritz1 parses generic types
   - ✅ Struct monomorphization: `Pair<i32>` → `Pair$i32` (2026-02-13)
   - ✅ Generic struct literals: `Pair<i32> { first: 1, second: 2 }` (2026-02-13)
   - ✅ Generic function calls: `gvec_new<i32>()` → `gvec_new$i32()`
   - [ ] Import resolution for ritzlib (Vec, Option, Result)

2. **Import resolution edge cases**
   - level4/01_ritzlib_str fails in ritz1
   - Need to debug import chain
   - RITZ_PATH environment variable not honored
...
| 2 | `ritz.lock` file generation | ✅ Complete |
| 2 | Dependency resolution algorithm | ✅ Complete (via Build System #109) |
| 2 | `ritz vendor` command | ✅ Complete |
...
## Freestanding/Kernel Target Support (#118)

Language extensions needed for the [Harland](https://github.com/ritz-lang/harland) microkernel.

### Required Features

| Feature | Priority | Complexity | Description |
|---------|----------|------------|-------------|
| `--target x86_64-none-elf` | P0 | Low | Freestanding compilation (no red zone, no stack protector) |
| `asm x86_64:` blocks | P0 | High | Inline assembly with variable interpolation |
| `@packed` attribute | P1 | Medium | Disable struct padding for hardware structures |
| `@naked` attribute | P1 | Medium | No function prologue/epilogue for ISR stubs |

### Implementation Plan

**Phase 1: Target Flag (Low-hanging fruit)**
- [ ] Add `--target` flag to ritz0
- [ ] Pass LLVM triple to llvmlite
- [ ] Disable stack protector (`-fno-stack-protector`)
- [ ] Disable red zone (`-mno-red-zone`)

**Phase 2: Inline Assembly**
- [ ] Lexer: `asm` keyword, architecture token (`x86_64`)
- [ ] Parser: `asm x86_64:` block with indented assembly lines
- [ ] Variable interpolation: `{varname}` → LLVM inline asm constraints
- [ ] Local labels: `.label` for jumps within block
- [ ] Emitter: Generate LLVM inline asm IR
...
**Phase 3: Struct Attributes**
- [ ] Parser: `@packed` before struct definition
- [ ] Emitter: Use LLVM `<{ ... }>` packed struct type
- [ ] Parser: `@naked` before function definition
- [ ] Emitter: Add `naked` attribute to LLVM function
...
*Last updated: 2026-02-13 (RFC #107 Phase 2 complete)*
