...
**Branch:** `main` (merged)
**Status:** ✅ Complete - Legacy Mode Removed
**Date:** 2026-02-14
...
**RERITZ is now the ONLY mode.** The `--syntax` flag has been removed. All code uses RERITZ syntax.

| Feature | Status | Notes |
|---------|--------|-------|
| `[[attr]]` attributes | ✅ Done | Standard attribute syntax |
...
| `@&x` mutable address-of | ✅ Done | Mutable reference |
...
| String redesign | ✅ Done | `"hello"` → StrView, `c"hello"` → *u8 (FFI) |
| Reference philosophy | ❌ Not started | `&T` → `@T` migration |
| `unsafe` blocks | ✅ Done | Parser/emitter support, test_unsafe.ritz |
| Legacy mode removal | ✅ Done | `--syntax` flag removed |

### String Literal Types

| Syntax | Type | Use Case |
|--------|------|----------|
| `"hello"` | `StrView` | Default - zero-copy string view (ptr+len) |
| `c"hello"` | `*u8` | FFI/syscalls - null-terminated C string |
| `s"hello"` | `Span<u8>` | Deprecated - use `"hello"` instead |

### Core API Functions

| Function | Signature | Notes |
|----------|-----------|-------|
| `prints(s)` | `fn prints(s: StrView)` | Print StrView to stdout |
| `eprints(s)` | `fn eprints(s: StrView)` | Print StrView to stderr |
| `prints_cstr(s)` | `fn prints_cstr(s: *u8)` | Print C string (deprecated) |
| `string_from(sv)` | `fn string_from(sv: StrView) -> String` | Convert StrView to owned String |
| `string_from_cstr(s)` | `fn string_from_cstr(s: *u8) -> String` | Convert C string to String (FFI) |
| `strview_to_cstr(s)` | `fn strview_to_cstr(s: *StrView) -> *u8` | Allocate null-terminated copy |

### Migration Status

The RERITZ migration is **complete**:

- ✅ All `@test` converted to `[[test]]` (883 tests across 95 files)
- ✅ All ritzlib modules updated to use StrView API
- ✅ `--syntax` flag removed from compiler
- ✅ Legacy mode conditionals removed from lexer/parser/emitter
- ✅ 35/36 ritzlib modules compile (testing.ritz has unrelated asm issue)
- ✅ All 209 compiler unit tests pass
...
- [x] `unsafe` blocks ✅ DONE
- [x] `--target x86_64-none-elf` ✅ DONE
