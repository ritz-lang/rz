...
# ritz1 needs ritz_crt0.o for _start entry point
...
run_ritz0() {
    ...
    # Compile LLVM IR to binary directly with clang
    if ! clang -O2 -nostartfiles -no-pie "$ll_file" -o "$bin_file" 2>/dev/null; then
        if [ $VERBOSE -eq 1 ]; then
            echo -e "  ${RED}clang error for ritz0 output${NC}"
        fi
        return 1
    fi
...
run_ritz1() {
    ...
    # Compile LLVM IR to binary with clang, linking ritz_crt0.o for _start
    if ! clang -O2 -nostartfiles -no-pie "$ll_file" "$RITZ1_CRT0" -o "$bin_file" 2>"$BUILD_DIR/${name}_ritz1_clang.err"; then
        if [ $VERBOSE -eq 1 ]; then
            echo -e "  ${RED}clang error for ritz1 output:${NC}"
            cat "$BUILD_DIR/${name}_ritz1_clang.err"
        fi
        return 1
    fi
