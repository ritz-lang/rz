...
class TokenType(Enum):
    ...
    TRAIT = auto()
    IMPL = auto()
    DYN = auto()           # dyn keyword for trait objects
    MUT = auto()
    ASYNC = auto()
    AWAIT = auto()
    LOOP = auto()
    ...
    # RERITZ: Issue #118 - Kernel/freestanding support
    ASM = auto()           # asm keyword for inline assembly blocks
    ...
    SLASH = auto()         # /
    PERCENT = auto()       # %
    DOLLAR = auto()        # $ (AT&T syntax immediate prefix in asm)
    AMP = auto()           # &
    PIPE = auto()          # |
    CARET = auto()         # ^
    TILDE = auto()         # ~
    ...
    # RERITZ: New tokens for syntax overhaul
    # See RERITZ.md for full specification
    LBRACKET2 = auto()     # [[ (attribute start)
    RBRACKET2 = auto()     # ]] (attribute end)
    COLON_AMP = auto()     # :& (mutable borrow parameter)
    COLON_EQ = auto()      # := (move/ownership parameter)
    AT_AMP = auto()        # @& (mutable reference type / take mut ref)
    ...
    # Attributes and address-of
    # NOTE: In RERITZ mode, @ means address-of, attributes use [[name]]
    # In legacy mode, @ is attribute prefix
    AT = auto()            # @ (legacy: attribute prefix, reritz: address-of)
