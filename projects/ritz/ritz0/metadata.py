...
def type_to_string(t: rast.Type) -> str:
    ...
    if isinstance(t, rast.NamedType):
        ...
        # Extract integer from size - may be int or IntLit expression
        if isinstance(t.size, int):
            size = t.size
        elif isinstance(t.size, rast.IntLit):
            size = t.size.value
        else:
            # Fallback for other expressions - use repr
            size = str(t.size)
        return f"[{size}]{type_to_string(t.inner)}"
