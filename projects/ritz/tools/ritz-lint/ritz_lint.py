...
If you see patterns that suggest the developer is working around language limitations rather than using proper Ritz idioms, flag them as ERRORS, not suggestions. The language has proper solutions — developers MUST use them.
...
Requires:
    ...
    # Review code in batches to avoid token limits
    ...
    for i in range(0, len(files), BATCH_SIZE):
        ...
        print(f"  Batch {batch_num}/{total_batches}: {len(batch)} files...", file=sys.stderr)

        batch_result = review_code(batch)
        all_issues.extend(batch_result.get("issues", []))
        summary = batch_result.get("summary", {})
        total_warnings += summary.get("warnings", 0)
        total_errors += summary.get("errors", 0)
        total_suggestions += summary.get("suggestions", 0)
