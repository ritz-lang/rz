...
## 2026-02-13: Phase 3-5 Feature Completion

### Boolean Filter Expressions
- [x] Added `and`, `or`, `not` operators to filter patterns
- [x] Recursive expression parser with correct precedence
- [x] Examples: `add and not slow`, `json or xml`, `not ignore`

### Attribute Filters
- [x] Added attribute markers via naming convention
- [x] `@slow` - matches tests with "slow" in name
- [x] `@ignore`/`@skip` - matches tests with "ignore" in name
- [x] `@fast` - matches tests NOT marked @slow
- [x] Combinable with booleans: `@fast and not @ignore`

### JUnit XML Output
- [x] Created `src/junit_reporter.ritz`
- [x] Compatible with Jenkins, GitLab CI, etc.
- [x] `<testsuite>`, `<testcase>`, `<failure>`, `<error>` elements
- [x] Times in seconds with 3 decimal places
- [x] XML entities properly escaped

### ANSI Color Support
- [x] TTY auto-detection using ioctl(TIOCGWINSZ)
- [x] `--color` to force colors on
- [x] `--no-color` to disable colors
- [x] Green for pass, red for fail/crash, yellow for timeout/skip
- [x] Dim dots, bold headers

### Documentation
- [x] Created comprehensive README.md
- [x] CLI reference with all options
- [x] Filter expression guide
- [x] Output format examples (default, JSON, JUnit)

### Test Count
- 24 tests total (21 original + 3 attribute examples)
