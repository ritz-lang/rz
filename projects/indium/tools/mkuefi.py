...
def run_step(label: str, cmd: list[str], timeout_s: int, trace: bool) -> None:
    start = time.time()
    log(f"start: {label}")
    if trace:
        log("cmd: " + " ".join(cmd))
...
def main() -> int:
    ...
    run_step("mkdir EFI", ["mmd", "-i", str(image), "::/EFI"], timeout_s, trace)
    run_step("mkdir EFI/BOOT", ["mmd", "-i", str(image), "::/EFI/BOOT"], timeout_s, trace)
    run_step("mkdir harland", ["mmd", "-i", str(image), "::/harland"], timeout_s, trace)
